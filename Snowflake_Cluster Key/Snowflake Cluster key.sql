...
•	What happens: Snowflake creates the logical structure for the my_large_table. 
At this point, no storage has been consumed because there is no data.

2. Insert Data:
Now, lets insert some data. For this example, imagine we are inserting data 
from multiple sources in a random order, not sorted by date or category.
...
However, because a cluster key is defined, Snowflakes automatic clustering service starts working in the background. 
...
By aligning physical storage with common filter patterns, I reduce scan time 
and improve cost efficiency—especially in analytics-heavy environments.”
