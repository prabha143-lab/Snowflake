Question:
How would you implement both row-level and column-level security in Snowflake so that users only see rows from their 
own department and sensitive columns like birth_date are masked unless the user is an admin?
...
RETURNS DATE ->
    CASE 
        WHEN CURRENT_ROLE() = 'ACCOUNTADMIN' THEN birth_date
        ELSE DATE_TRUNC('YEAR', birth_date)
    END;

ALTER TABLE employee_data 
MODIFY COLUMN birth_date 
SET MASKING POLICY mask_birth_year_only;
...
EMP_ID	FULL_NAME	DEPARTMENT	BIRTH_DATE	SALARY
101	Amit Sharma	HR	1990-01-01	50000

USE ROLE FINANCE_ROLE;
SELECT * FROM employee_data;

EMP_ID	FULL_NAME	DEPARTMENT	BIRTH_DATE	SALARY
102	Priya Mehta	Finance	1985-01-01	75000

USE ROLE IT_ROLE;
SELECT * FROM employee_data;

EMP_ID	FULL_NAME	DEPARTMENT	BIRTH_DATE	SALARY
103	Ravi Kumar	IT	1992-01-01	90000