...
Question 1 : In Snowflake, can you create permanent, transient, 
and temporary tables with the same name in the same schema and database?

Answer: You can create permanent and temporary tables with the same name in Snowflake, 
 but not permanent and transient tables with the same name in the same schema.

Question 2: In Snowflake, if a Transient table exists, can you 
create a Permanent table or a Temporary table with the same name 
in the same schema? What happens during querying?
...
However, you can create a Temporary table, which will 
shadow the Transient table in the current session.

Question 3: If a Temporary table is created first and data is inserted, 
can a Permanent table with the same name be created afterward? If so, 
where does subsequent data go, and what happens to the data when the session closes?

Answer: Yes, the Permanent table creation succeeds 
because Temporary tables are session-scoped and dont exist in the schema. 

However, all subsequent INSERT statements will still 
target the Temporary table due to name shadowing. Upon session close, 
the Permanent table remains but will be empty, as all data went into the ephemeral Temporary table.

Question 4: If a Temporary table is created first, 
can a Transient table with the same name be created afterward? 
Where does all the data reside, and what is the final 
state of the tables after the session closes?

Answer: Yes, the Transient table creation succeeds. 
All INSERT operations (both initial and subsequent) 
target the Temporary table due to shadowing. 
After the session closes, the Transient table remains 
in the schema but is empty, as the Temporary table captured all the inserted data.
